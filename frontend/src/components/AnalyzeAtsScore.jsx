  ...
  const handleDrop = (e) => {
    ...
    if (e.dataTransfer.files?.length > 0) {
     ...
     const Popup = ({ message, isError, onClose }) => (
            ...
            >
                <div className={`p-6 rounded-lg shadow-lg max-w-sm w-full ${isError ? 'bg-red-100 border border-red-200' : 'bg-green-100 border border-green-200'}`}>
                    <div className="flex justify-between items-center mb-4">
                        <p className={`font-semibold ${isError ? 'text-red-800' : 'text-green-800'}`}>{message}</p>
                        <button
                            onClick={onClose}
...
const OptimizerView = ({
  ...
  <div className="max-w-3xl mx-auto my-12 bg-gray-900 text-gray-200 rounded-3xl shadow-[0_10px_40px_rgba(0,0,0,0.5)] border border-gray-700 overflow-hidden">
    <div className="p-10 space-y-8">
      <header className="text-center space-y-2">
        <h2 className="text-4xl font-extrabold text-white tracking-tight">
          <Sparkles className="inline-block w-10 h-10 text-purple-400 mr-2 animate-pulse" />
          Resume Power-Up
        </h2>
        <p className="text-gray-400 max-w-lg mx-auto leading-relaxed">
          Elevate your resume by providing specific, measurable, and impactful
          details. Your answers will be used to craft powerful, metric-driven
        ...
        {questions.map((q, i) => (
          <div key={i} className="space-y-2">
            ...
            <input
              ...
              className="w-full mt-1 p-3 bg-gray-800 text-gray-100 border border-gray-700 rounded-lg placeholder-gray-500 focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition-all duration-300"
      ...
      {error && (
        <div className="text-sm text-red-400 text-center bg-red-900/50 border border-red-700 p-3 rounded-lg shadow-inner">
      ...
      <button
        ...
        className="w-full flex items-center justify-center gap-3 py-4 bg-purple-600 text-white text-lg font-bold rounded-xl shadow-lg hover:bg-purple-700 transition-all duration-300 transform hover:-translate-y-1 hover:scale-105 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2 focus:ring-offset-gray-900"
      >
      ...
      <div className="max-w-lg w-full bg-white p-8 rounded-xl shadow-2xl text-center space-y-4">
        {showResults ? (
          ...
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            className="bg-gradient-to-br from-white to-purple-50 p-8 rounded-lg"
          >
            ...
            <p className="text-gray-600 my-4">
              Your resume has been enhanced. Let's run a final check to see your
              new and improved score.
            </p>
            {error && (
            ...
            <button
              ...
              disabled={isChecking}
              className="w-full flex items-center justify-center gap-2 py-3 bg-purple-600 text-white font-semibold rounded-md hover:bg-purple-700 disabled:bg-purple-400 transition-all transform hover:scale-105"
            >
              {isChecking ? (
                <>
